...
public class OptionTest {
    public static void main(String[] args) throws Exception {
        ProcessTools.executeJavaProcess(PrependTestJavaOpts, "-XX:CompileCommand=option,package/class,ccstrlist,ControlIntrinsic,+_getClass", "-version")
        ...
        ProcessTools.executeJavaProcess(PrependTestJavaOpts, "-XX:CompileCommand=option,*,ccstrlist,ControlIntrinsic,+_getClass", "-version")
                    .shouldHaveExitValue(1)
                    .shouldContain("CompileCommand: An error occurred during parsing")
                    .shouldContain("Error: Did not specify any method name")
                    .shouldNotContain("# A fatal error has been detected by the Java Runtime Environment");
        ...
        ProcessTools.executeJavaProcess(PrependTestJavaOpts, "-XX:CompileCommand=option,*.ccstrlist,ccstrlist,ControlIntrinsic,+_getClass", "-version")
        ...
        ProcessTools.executeJavaProcess(PrependTestJavaOpts, "-XX:CompileCommand=option,class,PrintIntrinsics", "-version")
                    .shouldHaveExitValue(0)
                    .shouldNotContain("# A fatal error has been detected by the Java Runtime Environment");

        // corner case:
        // PrintIntrinsics could be a valid method name, so it is accepted in the well-formed case.
        ProcessTools.executeJavaProcess(PrependTestJavaOpts, "-XX:CompileCommand=option,class.PrintIntrinsics,PrintIntrinsics", "-version")
        ...
        ProcessTools.executeJavaProcess(PrependTestJavaOpts, "-XX:CompileCommand=dontinline,*::dontinline_*", "-version")
                    .shouldContain("CompileCommand: dontinline *.dontinline_* bool dontinline = true")
                    .shouldHaveExitValue(0)
                    .shouldNotContain("# A fatal error has been detected by the Java Runtime Environment");

        ProcessTools.executeJavaProcess(PrependTestJavaOpts, "-XX:CompileCommand=dontinline,*.dontinline", "-version")
